...
BEFORE_SENT=$(snapshot_counter "snmp_command_dispatched_total" 'device_name=~"e2e-tenant-A|e2e-tenant-B"')
...
if poll_until 45 5 "snmp_command_dispatched_total" 'device_name=~"e2e-tenant-A|e2e-tenant-B"' "$NEED_AT_LEAST"; then
    AFTER_SENT=$(snapshot_counter "snmp_command_dispatched_total" 'device_name=~"e2e-tenant-A|e2e-tenant-B"')
