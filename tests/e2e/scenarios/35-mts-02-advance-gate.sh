...
BEFORE_SENT=$(snapshot_counter "snmp_command_dispatched_total" 'device_name="e2e-tenant-P1"')
...
FRESH_BASELINE=$(snapshot_counter "snmp_command_dispatched_total" 'device_name="e2e-tenant-P1"')
# Poll for counter — SNMP SET round-trip + OTel export + Prometheus scrape takes time.
if poll_until 45 5 "snmp_command_dispatched_total" 'device_name="e2e-tenant-P1"' "$FRESH_BASELINE"; then
    AFTER_SENT_A=$(snapshot_counter "snmp_command_dispatched_total" 'device_name="e2e-tenant-P1"')
