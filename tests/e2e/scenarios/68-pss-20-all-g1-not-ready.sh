...
#   PSS-20c:   G2 log absence (sleep 10, --since=10s, G2_FOUND check)
...
if [ "$G2_FOUND" -eq 0 ]; then
    record_pass "PSS-20c: G2 not evaluated -- log absence (gate blocked -- G1 stale/Unresolved)" "G2 tier logs absent in 10s window"
else
