# Scenario 64: PSS-16 G1 mixed Resolved+Healthy -- gate passes, G2 evaluated
...
#                T2 all in-range -> Healthy (tier=3)
#                -> ALL G1 are Resolved or Healthy -> gate PASSES -> G2 tenants evaluated
#
# Per-OID values:
...
log_info "PSS-16: Re-priming all 4 tenants to healthy state..."
...
log_info "PSS-16: Waiting 8s for readiness grace (TimeSeriesSize=3, IntervalSeconds=1, GraceMultiplier=2 -> grace=6s)..."
sleep 8
...
# ---------------------------------------------------------------------------
# === PSS-16a: G1-T1 reaches tier=2 Resolved ===
# ---------------------------------------------------------------------------

log_info "PSS-16a: Polling for e2e-pss-g1-t1 tier=2 Resolved log..."
if poll_until_log 30 1 "e2e-pss-g1-t1.*tier=2" 15; then
    record_pass "PSS-16a: G1-T1 tier=2 Resolved" "log=tier2_G1T1_found"
else
    record_fail "PSS-16a: G1-T1 tier=2 Resolved" "tier=2 log for e2e-pss-g1-t1 not found within 30s"
fi

# ---------------------------------------------------------------------------
# === PSS-16b: G1-T2 stays at tier=3 Healthy ===
# ---------------------------------------------------------------------------

log_info "PSS-16b: Polling for e2e-pss-g1-t2 tier=3 Healthy log..."
if poll_until_log 30 1 "e2e-pss-g1-t2.*tier=3" 15; then
    record_pass "PSS-16b: G1-T2 tier=3 Healthy" "log=tier3_G1T2_found"
else
    record_fail "PSS-16b: G1-T2 tier=3 Healthy" "tier=3 log for e2e-pss-g1-t2 not found within 30s"
fi

# ---------------------------------------------------------------------------
# === PSS-16c: Gate passes -- G2-T3 shows tier=3 Healthy ===
...
log_info "PSS-16c: Polling for e2e-pss-g2-t3 tier=3 log (gate-pass assertion)..."
if poll_until_log 30 1 "e2e-pss-g2-t3.*tier=3" 15; then
    record_pass "PSS-16c: G2-T3 tier=3 (gate passed -- G1 mixed Resolved+Healthy)" "log=tier3_G2T3_found"
else
    record_fail "PSS-16c: G2-T3 tier=3 (gate passed -- G1 mixed Resolved+Healthy)" "tier=3 log for e2e-pss-g2-t3 not found within 30s -- gate may have blocked"
fi

# ---------------------------------------------------------------------------
# === PSS-16d: Gate passes -- G2-T4 shows tier=3 Healthy ===
# ---------------------------------------------------------------------------

log_info "PSS-16d: Polling for e2e-pss-g2-t4 tier=3 log (gate-pass assertion)..."
if poll_until_log 30 1 "e2e-pss-g2-t4.*tier=3" 15; then
    record_pass "PSS-16d: G2-T4 tier=3 (gate passed -- G1 mixed Resolved+Healthy)" "log=tier3_G2T4_found"
else
    record_fail "PSS-16d: G2-T4 tier=3 (gate passed -- G1 mixed Resolved+Healthy)" "tier=3 log for e2e-pss-g2-t4 not found within 30s -- gate may have blocked"
fi
