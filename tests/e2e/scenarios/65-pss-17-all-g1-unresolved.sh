...
#   PSS-17c:   G2 log absence (sleep 10, --since=10s, G2_FOUND check)
...
log_info "PSS-17c: Stabilizing -- waiting for gate-blocked cycle (2 evaluated)..."
...
if [ "$G2_FOUND" -eq 0 ]; then
    record_pass "PSS-17c: G2 not evaluated -- log absence (gate blocked -- all G1 Unresolved)" "G2 tier logs absent in 10s window"
else
