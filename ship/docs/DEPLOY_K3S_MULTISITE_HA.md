# SnmpCollector Offline Deployment Guide (K3s Multi-Site HA)

## Overview

This guide deploys the SnmpCollector system across **3 sites** using a single K3s **HA cluster**
spanning 3 server nodes. Each site runs a Linux VM on a Windows Server 2019 host via Hyper-V.
All 3 nodes run the K3s control plane (embedded etcd) — if any single node goes down,
the cluster remains operational. Multiple site namespaces share a single OTel Collector
DaemonSet (one per node). All required files are pre-included in the `ship/` folder —
...
|------|----------|---------|
| 6443 | TCP | K3s API server (server ↔ server, used by kubectl) |
| 2379 | TCP | etcd client requests (server ↔ server) |
| 2380 | TCP | etcd peer communication (server ↔ server) |
...
1. **Site A (first server):** 3.1 → 3.2 → 3.3 → 3.4 → 3.5 → 4A.1 → 4A.2 → 4A.3 → 4A.4 → 4A.5 → 4A.6
2. **Site B (join as server):** 3.2 → 3.3 → 3.4 → 3.5 → 4B.1 → 4B.2 → 4B.3 → 4B.4 → 4B.5 (needs Site A running first)
3. **Site C (join as server):** 3.2 → 3.3 → 3.4 → 3.5 → 4B.1 → 4B.2 → 4B.3 → 4B.4 → 4B.5 (needs Site A running first)

Step 3.1 (virtual switch) is done once. Each additional site repeats 3.2-3.5 (VM creation) then 4B (server join).

### Site naming plan

| Site | VM Name | VM IP | Ubuntu Server Name | K3s Role |
|------|---------|-------|--------------------|----------|
| A | simetra-k3s-a | 172.20.0.10 | simetra-k3s-a | server (first, --cluster-init) |
| B | simetra-k3s-b | 172.20.0.10 | simetra-k3s-b | server (join) |
| C | simetra-k3s-c | 172.20.0.10 | simetra-k3s-c | server (join) |
...
### Part 4A: Install K3s First Server (Site A only — one-time)
...
Run the installer in airgap mode with `--cluster-init` to enable embedded etcd for HA:
...
### Part 4B: Join K3s Server (Site B and Site C — one-time)

Repeat on each additional server site.
...
Run the installer as **server** joining the existing cluster.
...
```bash
INSTALL_K3S_SKIP_DOWNLOAD=true K3S_URL=https://<Site-A-IP>:6443 K3S_TOKEN=<node-token> ~/install.sh --server
```

> Note the `--server` flag at the end — this joins as a **server node** (not agent), running the
> full control plane with etcd. This is what enables HA.
>
> Replace `<Site-A-IP>` with the IP of Site A's VM that this server can reach over the network.
...
#### 4B.3 Configure kubectl on this server
...
#### 4B.4 Verify the server joined the cluster

From **any server node** (kubectl works on all server nodes in HA mode):

```bash
kubectl get nodes
```

Expected: the new server node appears in `Ready` status with `control-plane,master` role. After all 3 servers join:

```
NAME             STATUS   ROLES                  AGE
simetra-k3s-a    Ready    control-plane,master   10m
simetra-k3s-b    Ready    control-plane,master   2m
simetra-k3s-c    Ready    control-plane,master   1m
```

> **HA verification:** All 3 nodes show `control-plane,master`. If a node shows `<none>`,
> it joined as agent instead of server — re-run 4B.2 with the `--server` flag.

#### 4B.5 Clean up install files
...
All kubectl commands can run from **any server node** (HA mode). This guide uses Site A.
...
With all 3 nodes as servers (embedded etcd), the cluster tolerates **1 node failure**:

| Scenario | Impact | Recovery |
|----------|--------|----------|
| Any 1 node down | `kubectl` works from remaining nodes. Pods on surviving nodes stay running. Leader election continues — follower on another node acquires leadership within ~15s. | Start the VM. K3s auto-starts. Node rejoins. Preferred leader reclaims if configured. |
| 2 nodes down | **etcd loses quorum.** `kubectl` stops working. Existing pods keep running but no new scheduling, no lease renewals. | Start at least 1 of the 2 downed VMs to restore quorum. |
| All 3 down | Full outage. | Start all VMs. Cluster restores automatically. |

> **Key advantage over single-server:** If Site A (the first server) goes down, Sites B and C
> continue operating normally — `kubectl` works, pods schedule, leader election continues.
> With a single-server setup, losing the server node would stop all cluster operations.
...
On any server node (all 3 are servers in HA mode):
